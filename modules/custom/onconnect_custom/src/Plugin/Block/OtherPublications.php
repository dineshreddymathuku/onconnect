 ...
 *   admin_label = @Translation("Other Publications"),
 *   category = "Oncology"
 * )
 */
class OtherPublications extends BlockBase {
   ...
   * {@inheritdoc}
   */
  private function getPrimaryMenuTerms($vid = NULL) {
    $database = \Drupal::database();
    $tidsQuery = $database->select("taxonomy_term__field_show_in_primary_menu", "npm");
    $tidsQuery->fields("npm", ["entity_id"]);
    $tidsQuery->condition("bundle", $vid);
    $tidsQuery->condition("field_show_in_primary_menu_value", 1);
    $tids = $tidsQuery->execute()->fetchAll(\PDO::FETCH_ASSOC);
    $ids = array_column($tids, 'entity_id');
    return $ids;
  }
  ...
  public function build() {
    ...
    $vocabularyID = '';
    if ($route_name == "onconnect_custom.other_disease_areas") {
      $vocabularyID = "disease_areas";
      $paramsType = "disease-typed";
    }
    elseif ($route_name == "onconnect_custom.other_congresses") {
      $vocabularyID = "congresses";
      $paramsType = "congress-typed";
    }
    ...
    $terms_2 = \Drupal::entityTypeManager()
      ...
      ->loadTree($vocabularyID, 0, 1);

    if ($terms_2) {
      $ids = $this->getPrimaryMenuTerms($vocabularyID);

      foreach ($terms_2 as $term_2) {
        $term_load = Term::load($term_2->tid);

        if (in_array($term_2->tid, $ids)) {

          /*if ($term_load->hasField('field_group_header')) {
          if ($term_load->field_group_header->value) {
          $term_load_childrens = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadChildren($term_load->id());
          foreach ($term_load_childrens as $term_load_children) {
          $term_load_children_tid = $term_load_children->id();
          $tids[$term_load_children_tid] = $term_load_children_tid;
          }
          }
          }
          else {
          ...
          $tids[$term_2->tid] = $term_2->tid;
          $filterTerms[$term_2->tid] = [
            'id' => $term_2->tid,
            'value' => trim($term_2->name),
            'checked' => array_key_exists(str_replace(" ", "_", $paramsType . '-' . trim($term_2->name)), $params) ? "checked" : '',
          ];
        }

      }
    }
    $otherIndications = FALSE;
    $otherCongresses = FALSE;
    ...
    foreach ($params as $paramKey => $paramValue) {
      ...
      if (strpos($paramKey, 'disease-typed-') !== FALSE) {
        $diseaseTids[$paramValue] = $paramValue;
      }
      if (strpos($paramKey, 'congress-typed-') !== FALSE) {
        $congressTids[$paramValue] = $paramValue;
      }
    }

    if ($vocabularyID == "disease_areas" && empty($diseaseTids)) {
      $otherIndications = TRUE;
    ...
    elseif ($vocabularyID == "disease_areas" && $diseaseTids) {
      $otherIndications = TRUE;
      if ($productData && $indicationData) {

        $diseaseTids = $productData;
      }
      else {
        if ($productData) {
          $diseaseTids = $productData;
        ...
        if ($indicationData) {
          $diseaseTids = $indicationData;
        }
      }
      $nid->condition("field_related_disease_areas", $diseaseTids, "IN");
    }
    if ($vocabularyID == "congresses" && empty($congressTids)) {
      $otherCongresses = TRUE;
      $nid->condition("field_related_to_congresses", $tids, "IN");
    }
    elseif ($vocabularyID == "congresses" && $congressTids) {
      $otherCongresses = TRUE;
      $nid->condition("field_related_to_congresses", $congressTids, "IN");
    }
    if ($typesData) {
    ...
    if ($congressData) {
      if ($congressTids) {
        $congressData = array_combine($congressData, $congressTids);
      }
    ...
    foreach ($nids as $nid) {
      ...
      if ($diseaseAreaValues) {
        foreach ($diseaseAreaValues as $key => $diseaseAreaValue) {
          ...
          $child_terms = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadTree('disease_areas', $diseaseAreaValue['target_id'], 1);
          if ($diseaseAreaLoad && !$child_terms) {
            // If ($diseaseAreaLoad->parent->target_id) {.
            ...
            // If ($diseaseAreaLoad && $otherCongresses) {.
    ...
    foreach ($params as $paramKey => $paramValue) {
      ...
      if (strpos($paramKey, 'disease-typed-') !== FALSE) {
        ...
        $paramLinks[$filterKey] = [
          'value' => $paramValue,
          'label' => $filterTerms[$paramValue]['value'],
        ];
      }
      if (strpos($paramKey, 'congress-typed-') !== FALSE) {
    ...
    $build = [
      ...
      '#otherIndicatons' => $otherIndications,
      '#otherCongresses' => $otherCongresses,
      '#attached' => [
        'library' => [
